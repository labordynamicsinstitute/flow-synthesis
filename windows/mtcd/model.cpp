...
double* DoModel(double *data, double *new_data, int n,  
        ...
        int& betaAcc, int& gammaAcc, int& numberofmodels, int& B, int& G, 
		int UPPERLIMIT,MTRand& mt) {
    int i;
...
void CalculateRiskSummary(double *data, int n, double *riskmeasure, double *result, int UPPERLIMIT) {
	int *rt = new int[n];
	int *at = new int[n];
	int *arg_y = new int[n];
	int i,j;
...
void Run(string infile, int n, unsigned int seed, int niters, int burnin, int stride, string outfile, 
		 string syndatafile, string riskfile, int m, bool verbose, int UPPERLIMIT) {
	...
	double *models = DoModel(data,new_data, n, niters, burnin, stride, betaAcc, gammaAcc, L,B,G,UPPERLIMIT,mt);
	...
	for (int s= 0; s< m; s++) {
		...
		CalculateRiskSummary(new_data, n, riskmeasure, risksummary+s*(n+2),UPPERLIMIT);
	}
	if (verbose) {
		fclose(RISKMEASURE);
	}
	...
	fprintf(SDATA, "dt%d,Rall%d,Runq%d",m, m, m);
	fprintf(SDATA, "\n");
